...
export default function BiodiversityOverviewPage() {
  ...
  const { precinctId } = useParams()
  const searchedLocation = location.state?.searchedLocation || null
  ...
  const selectedArea = suburbs.find((suburb) => suburb.id === precinctId)
  useEffect(() => {
    let active = true
    if (!selectedArea?.id) {
  ...
  const selectSearchResult = (suburb, location) => {
    ...
    if (matchingArea?.id) navigate(`/biodiversity/${encodeURIComponent(matchingArea.id)}`, { state: { searchedLocation: location } })
  }
  const selectArea = (area, location = null) => navigate(`/biodiversity/${encodeURIComponent(area.id)}`, { state: { searchedLocation: location } })
  return (
    <main className="biodiversity-overview" id="area-selection">
      ...
      <aside className="overview-map" aria-label="Select a Melbourne precinct on the map"><SuburbOverviewMap suburbs={suburbs} selectedPrecinctId={precinctId || null} searchedLocation={searchedLocation} onSelect={selectArea} /></aside>
      {selectedArea && (
