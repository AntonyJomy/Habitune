...
type MapArea = { id: string; name: string; label?: string; positions: LatLngExpression[] | LatLngExpression[][] | LatLngExpression[][][]; summary: SuburbBiodiversitySummary }
type SuburbOverviewMapProps = { suburbs?: MapArea[]; selectedPrecinctId?: string | null; searchedLocation?: SearchLocation | null; onSelect: (area: MapArea, location: SearchLocation | null) => void }

export default function SuburbOverviewMap({ suburbs = [], selectedPrecinctId, searchedLocation, onSelect }: SuburbOverviewMapProps) {
  ...
  return <div className="suburb-map-shell">
    <MapContainer center={[-37.816, 144.958]} zoom={12} minZoom={11} maxZoom={14} scrollWheelZoom className="suburb-overview-map">
      ...
      {suburbs.filter((suburb) => Array.isArray(suburb.positions) && suburb.positions.length > 0).map((suburb) => {
        const isSelected = suburb.id === selectedPrecinctId
