...
export default function LocationSearch({ onChoose, suburbs = [] }) {
  ...
  const choose = (area) => { setError(''); setQuery(area.primary); setFocused(false); onChoose(area.suburb, area.searchedLocation) }
  const submit = async (event) => {
    ...
    setSearching(false)
    if (result.status === 'supported') { setFocused(false); return onChoose(result.suburb, result.searchedLocation) }
